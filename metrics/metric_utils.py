...
class MetricOptions:
    def __init__(self, G=None, lr_G=None, G_kwargs={}, dataset_kwargs={}, cond_dataset_kwargs=None, loader_kwargs=None,
        ...
        self.cond_dataset_kwargs      = None if cond_dataset_kwargs is None else dnnlib.EasyDict(cond_dataset_kwargs)
        self.loader_kwargs            = None if loader_kwargs is None else dnnlib.EasyDict(loader_kwargs)
...
def get_feature_detector(url, verbose=False):
    ...
    if key not in _feature_detector_cache:
        ...
        with dnnlib.util.open_url(url, verbose=(verbose and rank == 0)) as f:
...
def compute_feature_stats_for_dataset(
    ...
    if opts.cache:
        ...
        cache_file = dnnlib.make_cache_dir_path(
            "long-video-gan-metrics", pathlib.Path(dataset.dataset_dir).name,
    ...
    if use_image_dataset or opts.single_sample_per_video:
        ...
        data_iter = utils.get_infinite_data_iter(dataset, **loader_kwargs)
